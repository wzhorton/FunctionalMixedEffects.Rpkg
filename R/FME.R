...
#' @param v_fix fixed effect prior variance.
#'
...
#'
#' Pass the final model parameters and fit the model to data. \code{HyperParameter}
#' and \code{OutputConfig} objects must be defined.
#'
...
#' as columns and variables as rows (reverse of usual design matrices).
...
#'
#' @return Invisibly returns list of argument values. The resulting object \code{chains}
#' is saved in the \code{julia_obj} environment.
#' @export

mcmc_fme <- function(julia_obj, Yobs, Xfix, Xrand = NULL, p = 20){
  if(!all(is.double(Yobs))){
    stop("Yobs is not a matrix of doubles")
  }
  if(!all(is.double(Xfix))){
    stop("Xfix is not a matrix of doubles")
  }
  if(!all(is.double(Xrand)) && !is.null(Xrand)){
    stop("Xrand is neither NULL nor a matrix of doubles")
  }
  if(!is.integer(p)){
    stop("p is not a strict logical")
  }
...
#' Julia object are renamed using standard characters.
#' @export
...
#' @param Yobs,Xfix,Xrand,p see documentation for \code{mcmc_fme}
#' @param niter,nburn,nthin,save_random_effects see documentation for \code{set_config_fme}
#' @param a_sig,b_sig,a_tau,b_tau,a_lam,b_lam,v_fix see documentation for \code{set_hyperparm_fme}
#'
